...
public class XRDSFactory {
	 ...
	 */
	public XRDSFactory(DebugLogger log) {
	 ...
	 * type is invalid.
	 */
	public XRD resolveXRI(String xri, String serviceType)
		...
		URL callURL = new URL(this.xriProxyURL,
		...
		final ResponseBuffer respBuf = new ResponseBuffer();
		try {
			...
			String contentType = this.readContent(con, buf, respBuf);
			...
			// parse the XRD
			XRD xrd = this.parse(respBuf.getInputStream());
	 ...
	 */
	public XRDSHTTPDiscoveryResult discover(URL url)
		...
		try {
			...
			if (xrdsLocation != null) {
				...
				contentType = this.readContent(con, buf, respBuf);
				String md = respBuf.toString(
						this.getContentCharset(con, "UTF-8"));
				Matcher m = XRDS_LOCATION_LINK_PATTERN.matcher(md);
				if (m.find())
				...
				contentType = this.readContent(con, buf, respBuf);
				String html = respBuf.toString(
						this.getContentCharset(con, "UTF-8"));
				Matcher m = XRDS_LOCATION_META_PATTERN.matcher(html);
				if (m.find()) {
	 ...
	 * stream.
	 */
	public XRD parse(InputStream xrdsIn)
		throws SAXException, IOException {

		try {
			SAXParser parser = this.parserFactory.newSAXParser();

			XRDSParserHandler handler = new XRDSParserHandler();
	...
	private String readContent(final HttpURLConnection con, final byte[] buf,
		...
		if (debug) {
			...
			if (respBuf.size() > 0) {
				String charset = this.getContentCharset(con, "UTF-8");
		...
		if (contentType != null) {
			int semiInd = contentType.indexOf(';');
			if (semiInd > 0)
	 ...
	 */
	private String getContentCharset(HttpURLConnection con,
